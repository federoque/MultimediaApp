...
import { getContentById, loadingOn, updateContent } from "../actions"
...
export default function UpdateContent(){
    ...
    function handleSubmit(e){
        e.preventDefault()
        dispatch(updateContent(accesstoken, {name: input.name, url: input.url, img: input.img, categories: input.categories}, params.id))
        setInput(prev=>{
            return{
                name: "",
                url:"",
    ...
    if(role === 'Admin'){
        ...
        return(
            ...
            <form className={s.form}>
                ...
                <button onClick={handleSubmit} disabled = {(input.name=== "" || input.url==="" || input.img=== "" || input.categories.length===0) ? true : false}>Modificar Contenido</button>
            </form>
            <button onClick={()=> navigate('/content/'+params.id)}>Atras</button>
            </div>
        )
    }
}